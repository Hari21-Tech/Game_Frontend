...
function Leaderboard() {
  ...
  if (!data.length) return <div className="text-gray-400">No leaderboard data</div>;
...
function Announcement() {
  ...
  if (loading) return <div className="text-gray-400">Loading...</div>;
  if (error) return <div className="text-red-400">{error}</div>;
  if (!announcement) return <div className="text-gray-400">No announcements</div>;

  return <div className="text-white">{announcement}</div>;
}

export default function PauseOverlay({ onClose, targetDate }: { onClose: () => void; targetDate: string }) {
  return (
    <div className="fixed inset-0 z-50 bg-background/90 flex items-center justify-center">
      <div className="max-w-2xl w-full rounded-2xl shadow-2xl bg-background border border-border p-12 relative flex flex-col gap-12">
        ...
        <h2 className="text-3xl font-bold text-foreground mb-8 text-center tracking-tight">Paused</h2>
